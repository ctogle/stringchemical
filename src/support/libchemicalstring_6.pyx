...
#given a set of points and a domain value, use interpolation to sample the function
class external_signal(object):
    # this assumes a monotonically increasing domain

    #def __init__(self, data = [(0,1),(1,4),(2,-2),(3,1)]):
    def __init__(self, datastr):
        xdatastr,ydatastr = datastr.split('@')

        xdatastr = xdatastr.split('$')
        xdatastr.pop()
        ydatastr = ydatastr.split('$')
        ydatastr.pop()
...
def generate_functions(count_targets, functions_string):
    ...
    def check_ext_signals(string):

        global extsig1

        if string.count('external_signal('):
            extname = 'extsignal1'
            extdex = string.find('external_signal(') + 16
            datastr = string[extdex:]
            datastr = datastr[:datastr.find(',')]
            extsig1 = external_signal(datastr)

            string = string.replace(datastr + ',', '')
            string = string.replace('external_signal',extname)

        return string

        # before converting, make global func for any new ext signal
        # remove the first argument to the function after replacing
        # "external_signal" with said new global func
        # convert then just turns the domain into counts[x]
    encoded_functions = [check_ext_signals(string) for string in encoded_functions]
